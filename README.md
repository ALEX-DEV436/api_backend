...
<p>Bem-vindos ao meu projeto!
  ...
  partindo da premissa de um dev (com certa experiência) e, que esta começando a trabalhar com essas ferramentas agora. Oportunidade unica!!! Acompanhe todo processo que utilizo quando fico frente a frenta com uma proposta nova de trabalho.<br />
  Vamos desenvolver uma API utilizando o Node.js para back-end e o React no front-end. Nossa (API) devera comunicar com uma API externa - (utilizaremos a da star - wars) para conseguir responder conforme seja solicitada pelo usuario com acesso ao nosso sistema. Esta aplicação devera seguir as diretrizes REST,  garantindo que a comunicação entre o servidor e o cliente seja super suave e padronizada. E um adendo, apesar de desenvolver aplicações web a muitos anos eu, nunca trabalhei com estas ferramentas.Ou seja, a portunidade de quem tá começando acompanhar o inicio de uma viagem na construção de uma aplicação completa, vamos trabalhar na frente e nos bastidores de uma aplicação, resolveremos um problema e  no final vamos aprender novos conhecimentos super uteis no meercado de trabalho.Animados!Espero que sim! voce vai vivencia uma experiencia unica e empolgante, a inquietude na vida de um desenvolvedor Full Stack, não estamos presos (digo restritamente, pois temos que entender,os principios que regem cada tema, não dá para criar um site e colocar o texto amarelo em por cima de um vermelho citilante ) a nenhuma linguagem, nem a conceitos de designer, somos livres para desenvolvermos o que der na telha, antes deste projeto eu já estava criando um game com c# e unity logo coloco aqui no git Hub, agora vou focar neste projeto. Venha comigo nesta jornada e vamos juntos descobrir as possibilidades e oportunidades que estas novas ferramentas (digo para mim) podem oferecer.Vai ser divertido!!&nbsp;&nbsp;let's go!  
</p>
